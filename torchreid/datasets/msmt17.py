...
class MSMT17(BaseImageDataset):
    ...
    def __init__(self, root='data', verbose=True, **kwargs):
        ...
        self.root = '/import/sgg-homes/sgg-share/re-id_datasets/image_sets/'
        ...
        if args.CombineAll:
            train += val
            query = self._process_dir_CombineAll(self.test_dir, self.list_query_path)
            gallery = self._process_dir_CombineAll(self.test_dir, self.list_gallery_path)
            train += query
            train += gallery
        else:
    ...
    def _process_dir_CombineAll(self, dir_path, list_path):
        with open(list_path, 'r') as txt:
            lines = txt.readlines()
        dataset = []
        pid_container = set()
        for img_idx, img_info in enumerate(lines):
            img_path, pid = img_info.split(' ')
            pid = int(pid)+1041 # need to relabel, avoid overlap with train data label
            camid = int(img_path.split('_')[2]) - 1 # index starts from 0
            img_path = osp.join(dir_path, img_path)
            dataset.append((img_path, pid, camid))
            pid_container.add(pid)
