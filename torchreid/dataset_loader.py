...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.is_train:
            # img_raw, img_path, pid, camid = self.dataset[index]
            # img = img_raw #that is to load dataset in the memory
        ...
        if self.transform is not None:
            if self.is_train:
                if args.Not_use_RS:
                    ...
                    return img, pid, camid, img_path, img
                else:
                    img, GT = self.transform(img)
                    return img, pid, camid, img_path, GT
            else:
                img = self.transform(img)
