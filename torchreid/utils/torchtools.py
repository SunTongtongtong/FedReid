...
def close_specified_layers(model, close_layers):
    """
    Close specified layers in model for training while keeping
    other layers un-frozen.

    Args:
    - model (nn.Module): neural net model.
    - close_layers (list): list of layer names.
    """
    if isinstance(model, nn.DataParallel):
        model = model.module
