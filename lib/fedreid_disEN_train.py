...
def FedReID_disEN_train(model, model_exp, w_glob, opt, local_datasets, dict_users, dataloaders_val):
    ...
    #shitong
    writer = SummaryWriter('runs/disentangle_experiment_2')

    model_pth = 'modelTest_{}_{}.pth'.format(opt.agg, opt.frac) # saved model
    ...
    w_expert = model_exp.cpu().state_dict()
    for i in range(opt.nusers):
        w_all.append(w_expert) # initial
    w_all.append(w_glob)
    ...
    for epoch in range(num_epochs):
        ...
        w_locals, loss_locals = [], []# aggreated local model parameters and local loss
        ...
        for idx in idxs_users_selected:
            ...
            model_exp.load_state_dict(weights_disEN_aggregate(w_tmp, opt.dp, opt.alpha_mu, idx_client=idx, is_local=True)) # local model parameter update
            ...
            out_dict = local.update_weights_disEN(model=copy.deepcopy(model_exp), cur_epoch=epoch,
            ...
            #add loss
            writer.add_scalar('result/client {} id loss'.format(idx),
                              out_dict['id_loss'],
                              epoch)

            writer.add_scalar('result/client {} mse loss'.format(idx),
                              out_dict['mse_loss'],
                              epoch)
        ...
        if opt.agg == 'avg': # current version  only supports modified federated average strategy
            w_glob = weights_disEN_aggregate(w_locals, opt.dp, opt.alpha_mu, idx_client=idxs_users_selected, is_local=False, global_model=w_all[-1]) #, kl_loss_locals = kl_loss_locals  central model parameter update
        model.load_state_dict(w_glob)

        print('-' * 20)
        print('All Client {} Loss: {:.4f}'.format('Training', sum(loss_locals) / len(loss_locals)))
        writer.add_scalar('All client loss',
                          sum(loss_locals) / len(loss_locals),
                          epoch)

        # current model evaluation
        #val_loss = local.evaluate(data_loader=dataloaders_val['val'], model=model,disEN=True)
        ...
        if epoch%10 ==0:
            with open(model_saved+str(epoch), 'wb') as f:
                torch.save(model.state_dict(), f)
