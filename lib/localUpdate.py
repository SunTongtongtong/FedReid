...
class LocalUpdateLM(object):
    def __init__(self, args, dataset, idxs, nround, user,n_data):
        self.args = args
        self.round = nround
        self.user = user
       ...
       #shitong
        ...
        for epoch in range(self.args.local_ep):
            ...
            for data in tqdm(self.data_loader):
                ...
                # forward
                outputs,feat = model(inputs, idx_client)
                outputs_sv,feat_sv = model_sv(inputs, idx_client)

                # compute loss
                _, preds = torch.max(outputs.data, 1)
                loss_local = self.criterion_ce(outputs, labels) # local model loss
