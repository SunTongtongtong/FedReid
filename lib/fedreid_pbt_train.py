...
def FedReID_pbt_train(model, w_glob, opt, local_datasets, dict_users, dataloaders_val):
    ...
    for i in range(opt.nusers + 1):
        w_all.append(w_glob)  # initial  -> w_all: 4 clients + global

    for i in range(opt.PBT_num):
        w_overAll.append(w_all)
    ...
    for epoch in range(num_epochs):
        ...
        for idx_w_all, w_all in enumerate(w_overAll):
            ...
            for idx in idxs_users_selected:
                ...
                local = LocalUpdateLM(args=opt, dataset=local_datasets[idx], idxs=dict_users[idx][0], nround=epoch,
                                      user=idx)

                # local client weight update

                # shitong reidentify w_glob
                w_glob = w_all[opt.nusers]
                w_tmp.append(w_glob)  # server model parameters
                w_tmp.append(w_all[idx])  # local model parameters
                model.load_state_dict(weights_aggregate(w_tmp, opt.dp, opt.alpha_mu, idx_client=idx,
                                                        is_local=True))  # local model parameter update
            ...
            if opt.agg == 'avg':  # current version  only supports modified federated average strategy
                w_glob = weights_aggregate(w_locals, opt.dp, opt.alpha_mu, idx_client=idxs_users_selected,
                                           is_local=False)  # central model parameter update
                w_all[opt.nusers] = w_glob     # todo: when do we use deep copy
            model.load_state_dict(w_glob)

            w_overAll[idx_w_all]= w_all

        print('-' * 20)
        print('PBT loop finished')
