...
class ChannelGate_SpatialGate_parallel(nn.Module):
    """A mini-network that generates channel-wise gates conditioned on input tensor."""

    def __init__(self, in_channels, num_gates=None, return_gates=False,
                 gate_activation='sigmoid', reduction=16, layer_norm=False):
        super(ChannelGate_SpatialGate_parallel, self).__init__()
        if num_gates is None:
            num_gates = in_channels
        self.return_gates = return_gates
        self.global_avgpool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        #Spatial attention:
        _, _, spatial_weight = self.SpatialGate(x)
        ...
        if self.norm1 is not None:
            x = self.norm1(x)
        x = self.relu(x)
        channel_weight = self.fc2(x)
        if self.gate_activation is not None:
            channel_weight = self.gate_activation(channel_weight)
        if self.return_gates:
            return channel_weight
        return input * channel_weight * spatial_weight, input * (1 - channel_weight * spatial_weight), channel_weight * spatial_weight
...
class ResNet_SNR(nn.Module):
    ...
    def __init__(self, loss, block, layers,
        ...
        if args.CBAM:
            self.style_reid_laye1 = CBAM(256, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=False)

            self.style_reid_laye2 = CBAM(512, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=False)

            self.style_reid_laye3 = CBAM(1024, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=False)

            self.style_reid_laye4 = CBAM(2048, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=False)
        elif args.CBAM_S:
            self.style_reid_laye1 = CBAM(256, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=True)

            self.style_reid_laye2 = CBAM(512, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=True)

            self.style_reid_laye3 = CBAM(1024, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=True)

            self.style_reid_laye4 = CBAM(2048, reduction_ratio=args.ratio, pool_types=['avg'], no_channel=True)
        elif args.C_S_att_parallel:
            self.style_reid_laye1 = ChannelGate_SpatialGate_parallel(256, num_gates=256, return_gates=False,
                     gate_activation='sigmoid', reduction=args.ratio, layer_norm=False)
            self.style_reid_laye2 = ChannelGate_SpatialGate_parallel(512, num_gates=512, return_gates=False,
                     gate_activation='sigmoid', reduction=args.ratio, layer_norm=False)
            self.style_reid_laye3 = ChannelGate_SpatialGate_parallel(1024, num_gates=1024, return_gates=False,
                     gate_activation='sigmoid', reduction=args.ratio, layer_norm=False)
            self.style_reid_laye4 = ChannelGate_SpatialGate_parallel(2048, num_gates=2048, return_gates=False,
                     gate_activation='sigmoid', reduction=args.ratio, layer_norm=False)
        else:
        ...
        if args.pool_conv:
            self.fc1_IN = nn.Conv2d(256, 256, kernel_size=1, bias=True, padding=0)
            self.fc2_IN = nn.Conv2d(512, 512, kernel_size=1, bias=True, padding=0)
            self.fc3_IN = nn.Conv2d(1024, 1024, kernel_size=1, bias=True, padding=0)
            #self.fc4_IN = nn.Conv2d(2048, 2048, kernel_size=1, bias=True, padding=0)
            self.fc1_useful = nn.Conv2d(256, 256, kernel_size=1, bias=True, padding=0)
            self.fc2_useful = nn.Conv2d(512, 512, kernel_size=1, bias=True, padding=0)
            self.fc3_useful = nn.Conv2d(1024, 1024, kernel_size=1, bias=True, padding=0)
            #self.fc4_useful = nn.Conv2d(2048, 2048, kernel_size=1, bias=True, padding=0)
            self.fc1_useless = nn.Conv2d(256, 256, kernel_size=1, bias=True, padding=0)
            self.fc2_useless = nn.Conv2d(512, 512, kernel_size=1, bias=True, padding=0)
            self.fc3_useless = nn.Conv2d(1024, 1024, kernel_size=1, bias=True, padding=0)
            #self.fc4_useless = nn.Conv2d(2048, 2048, kernel_size=1, bias=True, padding=0)
    ...
    def forward(self, x):
        ...
        v_4_ori = self.global_avgpool(f_4_ori).view(f_4_ori.size(0), -1)
        v_3_ori = self.global_avgpool(f_3_ori).view(f_3_ori.size(0), -1)
        v_2_ori = self.global_avgpool(f_2_ori).view(f_2_ori.size(0), -1)
        v_1_ori = self.global_avgpool(f_1_ori).view(f_1_ori.size(0), -1)

        if args.pool_conv:
            #v_IN_4 = self.fc4_IN(self.global_avgpool(f_IN_4)).view(f_IN_4.size(0), -1)
            v_IN_4 = self.global_avgpool(f_IN_4).view(f_IN_4.size(0), -1)
            v_IN_3 = self.fc3_IN(self.global_avgpool(f_IN_3)).view(f_IN_3.size(0), -1)
            v_IN_2 = self.fc2_IN(self.global_avgpool(f_IN_2)).view(f_IN_2.size(0), -1)
            v_IN_1 = self.fc1_IN(self.global_avgpool(f_IN_1)).view(f_IN_1.size(0), -1)
        else:
            v_IN_4 = self.global_avgpool(f_IN_4).view(f_IN_4.size(0), -1)
        ...
        if args.pool_conv:
            ...
            v3 = self.fc3_useful(v3).view(v3.size(0), -1)
            v2 = self.global_avgpool(f_2)
            v2 = self.fc2_useful(v2).view(v2.size(0), -1)
            v1 = self.global_avgpool(f_1)
            v1 = self.fc1_useful(v1).view(v1.size(0), -1)
        else:
            v3 = self.global_avgpool(f_3)
        ...
        if args.pool_conv:
            ...
            #v4_useless = self.fc4_useless(v4_useless).view(v4_useless.size(0), -1)
            v4_useless = v4_useless.view(v4_useless.size(0), -1)
            v3_useless = self.global_avgpool(f_3_reid_useless)
            v3_useless = self.fc3_useless(v3_useless).view(v3_useless.size(0), -1)
            v2_useless = self.global_avgpool(f_2_reid_useless)
            v2_useless = self.fc2_useless(v2_useless).view(v2_useless.size(0), -1)
            v1_useless = self.global_avgpool(f_1_reid_useless)
            v1_useless = self.fc1_useless(v1_useless).view(v1_useless.size(0), -1)
        else:
            v4_useless = self.global_avgpool(f_4_reid_useless)
            v4_useless = v4_useless.view(v4_useless.size(0), -1)
            v3_useless = self.global_avgpool(f_3_reid_useless)
        ...
        if not self.training:
            # if args.Vis_actMap or args.Vis_KLMap:
            #     return v, f_4, f_3, f_2, f_1, \
            #        f_4_reid_useless, f_3_reid_useless, f_2_reid_useless, f_1_reid_useless, \
            #        f_IN_4, f_IN_3, f_IN_2, f_IN_1, \
            #        f_4_ori, f_3_ori, f_2_ori, f_1_ori, \
            #        f_style_4_reid_useful, f_style_3_reid_useful, f_style_2_reid_useful, f_style_1_reid_useful, \
            #        f_style_4_reid_useless, f_style_3_reid_useless, f_style_2_reid_useless, f_style_1_reid_useless
            # elif args.Vis_tSNEMap:
            #     return v, \
            #        v_4_ori, v_3_ori, v_2_ori, v_1_ori, \
            #        v4, v3, v2, v1, \
            #        v4_useless, v3_useless, v2_useless, v1_useless, \
            #        v_IN_4, v_IN_3, v_IN_2, v_IN_1, \
            #        self.global_avgpool(f_style_4_reid_useful).view(f_style_4_reid_useful.size(0), -1), \
            #        self.global_avgpool(f_style_3_reid_useful).view(f_style_3_reid_useful.size(0), -1), \
            #        self.global_avgpool(f_style_2_reid_useful).view(f_style_2_reid_useful.size(0), -1), \
            #        self.global_avgpool(f_style_1_reid_useful).view(f_style_1_reid_useful.size(0), -1), \
            #        self.global_avgpool(f_style_4_reid_useless).view(f_style_4_reid_useless.size(0), -1), \
            #        self.global_avgpool(f_style_3_reid_useless).view(f_style_3_reid_useless.size(0), -1), \
            #        self.global_avgpool(f_style_2_reid_useless).view(f_style_2_reid_useless.size(0), -1), \
            #        self.global_avgpool(f_style_1_reid_useless).view(f_style_1_reid_useless.size(0), -1)
            # elif args.Vis_selective_weights:
            #     return selective_weight_useful_4, selective_weight_useful_3, selective_weight_useful_2, selective_weight_useful_1
            # else:
            return v
...
def resnet50_SNR( loss, pretrained='imagenet', **kwargs):
    model = ResNet_SNR(
        loss=loss,
        block=Bottleneck,
        layers=[3, 4, 6, 3],
